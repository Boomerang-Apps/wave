    ...
    <div className="max-w-[1440px] mx-auto pb-12">
      ...
      <div className="flex items-center justify-between py-4 mb-6 border-b border-border">
        ...
        <div className="flex items-center gap-3">
          <span className="px-3 py-1 bg-green-100 text-green-700 rounded-full text-xs font-medium">7 Agents</span>
          <span className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-medium">8 Gates</span>
          <span className="px-3 py-1 bg-purple-100 text-purple-700 rounded-full text-xs font-medium">DO-178C Inspired</span>
        </div>
      </div>
        ...
        {activeTab === 'overview' && (
          <>
            ...
            <div className="grid grid-cols-6 gap-4">
              {[
                { label: 'Agents', value: '7', color: 'bg-blue-50 text-blue-600' },
                { label: 'Gates', value: '8', color: 'bg-purple-50 text-purple-600' },
                { label: 'Phases', value: '6', color: 'bg-green-50 text-green-600' },
                { label: 'Approval Levels', value: '6', color: 'bg-orange-50 text-orange-600' },
                { label: 'Domains', value: '4', color: 'bg-cyan-50 text-cyan-600' },
                { label: 'Forbidden Ops', value: '108', color: 'bg-red-50 text-red-600' },
              ].map((stat) => (
            ...
            <div className="bg-white border border-border rounded-2xl p-6">
              ...
              <div className="bg-zinc-50 rounded-xl p-6 font-mono text-sm overflow-x-auto">
...
└─────────────────────────────────────────────────────────────────────────────────┘
        ...
        {activeTab === 'agents' && (
          <>
            ...
            <div className="bg-gradient-to-b from-zinc-900 to-zinc-800 rounded-2xl p-8 text-white mb-6">
              <h4 className="text-lg font-semibold mb-6 text-zinc-300">Agent Hierarchy</h4>
              <div className="flex flex-col items-center gap-4">
                <div className="px-6 py-3 bg-amber-500/20 border border-amber-500/50 rounded-xl text-amber-300 font-medium">
                  Human (L1 Approval)
                </div>
                <div className="w-px h-6 bg-zinc-600" />
                <div className="px-6 py-3 bg-purple-500/20 border border-purple-500/50 rounded-xl text-purple-300 font-medium">
                  CTO Agent (Opus 4.5) - L2
                </div>
                <div className="w-px h-6 bg-zinc-600" />
                <div className="px-6 py-3 bg-blue-500/20 border border-blue-500/50 rounded-xl text-blue-300 font-medium">
                  PM Agent (Opus 4.5) - L3
                </div>
                <div className="w-px h-6 bg-zinc-600" />
                <div className="flex gap-4">
                  <div className="px-4 py-2 bg-green-500/20 border border-green-500/50 rounded-lg text-green-300 text-sm">FE-Dev-1</div>
                  <div className="px-4 py-2 bg-green-500/20 border border-green-500/50 rounded-lg text-green-300 text-sm">FE-Dev-2</div>
                  <div className="px-4 py-2 bg-cyan-500/20 border border-cyan-500/50 rounded-lg text-cyan-300 text-sm">BE-Dev-1</div>
                  <div className="px-4 py-2 bg-cyan-500/20 border border-cyan-500/50 rounded-lg text-cyan-300 text-sm">BE-Dev-2</div>
                </div>
                <div className="w-px h-6 bg-zinc-600" />
                <div className="flex gap-4">
                  <div className="px-6 py-3 bg-orange-500/20 border border-orange-500/50 rounded-xl text-orange-300 font-medium">
                    QA Agent (Haiku 4) - L4
                  </div>
                  <div className="px-6 py-3 bg-red-500/20 border border-red-500/50 rounded-xl text-red-300 font-medium">
                    Dev-Fix (Gate 4.5)
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                { name: 'CTO', model: 'Opus 4.5', role: 'Chief Technology Officer', gates: 'Pre-flight, Escalations', color: 'bg-purple-500', desc: 'Architecture decisions, technical oversight, schema approvals, L2 decisions' },
                { name: 'PM', model: 'Opus 4.5', role: 'Project Manager', gates: '0, 5-6, 7', color: 'bg-blue-500', desc: 'Story assignment, orchestration, merge approvals, L3 decisions' },
                { name: 'FE-Dev-1', model: 'Sonnet 4', role: 'Frontend Developer (Wave 1)', gates: '2-3', color: 'bg-green-500', desc: 'Implements Wave 1 frontend features in isolated worktree' },
                { name: 'FE-Dev-2', model: 'Sonnet 4', role: 'Frontend Developer (Wave 2)', gates: '2-3', color: 'bg-green-600', desc: 'Implements Wave 2 frontend features (may depend on Wave 1)' },
                { name: 'BE-Dev-1', model: 'Sonnet 4', role: 'Backend Developer (Wave 1)', gates: '2-3', color: 'bg-cyan-500', desc: 'Implements Wave 1 backend/API features in isolated worktree' },
                ...
                { name: 'QA', model: 'Haiku 4', role: 'Quality Assurance', gates: '4', color: 'bg-orange-500', desc: 'Validation (build, typecheck, lint, test), acceptance criteria, L4 review' },
                { name: 'Dev-Fix', model: 'Sonnet 4', role: 'Bug Fix Agent', gates: '4.5', color: 'bg-red-500', desc: 'Handles retry loop when QA rejects (max 3 retries before escalation)' },
              ].map((agent) => (
                <div key={agent.name} className="bg-white border border-border rounded-xl p-5 hover:border-primary/50 transition-colors">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-3 pt-3 border-t border-border flex items-center gap-2">
                        ...
                        <span className="px-2 py-0.5 bg-blue-100 text-blue-700 rounded text-xs font-medium">{agent.gates}</span>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <>
                ...
                {[
                  ...
                  <div key={g.gate} className="flex items-center">
                    <div className={cn(
                      ...
                      g.status === 'pass' ? 'bg-green-500' :
                      g.status === 'active' ? 'bg-blue-500 ring-4 ring-blue-500/30' :
                      'bg-zinc-700'
                    )}>
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={g.gate} className="bg-white border border-border rounded-xl p-5">
                  <div className="flex items-start gap-4">
                    <div className="w-14 h-14 bg-blue-100 rounded-xl flex items-center justify-center">
                      <span className="font-bold text-blue-600">G{g.gate}</span>
                    </div>
                    <div className="flex-1">
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-6">
              {[
                ...
                <div key={domain.id} className="bg-white border border-border rounded-xl overflow-hidden">
                  ...
                  <div className="p-5 space-y-4">
                    ...
                    <div>
                      <p className="text-xs font-medium text-green-600 uppercase tracking-wide mb-2">Owned Paths</p>
                      <div className="space-y-1">
                        {domain.owned.map((p) => (
                          <code key={p} className="block text-xs bg-green-50 text-green-700 px-2 py-1 rounded font-mono">{p}</code>
                        ))}
                      </div>
                    </div>
                    <div>
                      <p className="text-xs font-medium text-red-600 uppercase tracking-wide mb-2">Forbidden Paths</p>
                      <div className="space-y-1">
                        {domain.forbidden.map((p) => (
                          <code key={p} className="block text-xs bg-red-50 text-red-700 px-2 py-1 rounded font-mono">{p}</code>
                        ))}
                      </div>
                    </div>
                  </div>
          ...
          <>
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={p.phase} className="bg-white border border-border rounded-xl p-5">
                  <div className="flex items-start gap-4">
                    <div className="w-14 h-14 bg-purple-100 rounded-xl flex items-center justify-center">
                      <span className="font-bold text-purple-600">P{p.phase}</span>
                    </div>
                    <div className="flex-1">
        ...
        {activeTab === 'approval' && (
          <>
            ...
            <div className="space-y-4">
              {[
                { level: 'L0', name: 'FORBIDDEN', approver: 'NONE', color: 'bg-red-500', desc: 'Never allowed - 108 forbidden operations', examples: ['DROP DATABASE', 'rm -rf', 'git push --force', 'cat .env'] },
                ...
                { level: 'L2', name: 'CTO APPROVAL', approver: 'CTO Agent', color: 'bg-purple-500', desc: 'CTO agent can approve', examples: ['New modules', 'Shared interfaces', 'API response formats', 'Schema changes'] },
                { level: 'L3', name: 'PM APPROVAL', approver: 'PM Agent', color: 'bg-blue-500', desc: 'PM agent can approve', examples: ['Story assignment', 'Gate transitions', 'Cross-domain coordination'] },
                { level: 'L4', name: 'QA REVIEW', approver: 'QA Agent', color: 'bg-orange-500', desc: 'QA agent validates', examples: ['Code quality', 'Test validation', 'Acceptance criteria'] },
                { level: 'L5', name: 'AUTO-ALLOWED', approver: 'NONE', color: 'bg-green-500', desc: 'Automatically allowed within domain', examples: ['Read files in domain', 'Write to assigned domain', 'Run tests', 'Commit to feature branch'] },
              ].map((l) => (
                <div key={l.level} className="bg-white border border-border rounded-xl overflow-hidden">
                  <div className={cn("px-5 py-3 text-white flex items-center justify-between", l.color)}>
                    ...
                    <span className="px-3 py-1 bg-white/20 rounded-full text-sm">{l.approver}</span>
                  </div>
                  <div className="p-5">
  ...
  "timestamp": "2026-01-16T10:30:00Z"
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                { name: 'Gate 3 Complete', file: 'signal-wave{N}-gate3-{agent}-complete.json', creator: 'FE/BE Dev', color: 'bg-green-100 text-green-700' },
                { name: 'Gate 4 Approved', file: 'signal-wave{N}-gate4-approved.json', creator: 'QA', color: 'bg-green-100 text-green-700' },
                { name: 'Gate 4 Rejected', file: 'signal-wave{N}-gate4-rejected.json', creator: 'QA', color: 'bg-red-100 text-red-700' },
                { name: 'Gate 4.5 Retry', file: 'signal-wave{N}-gate4.5-retry.json', creator: 'Orchestrator', color: 'bg-orange-100 text-orange-700' },
                { name: 'Gate 4.5 Fixed', file: 'signal-wave{N}-gate4.5-fixed.json', creator: 'Dev-Fix', color: 'bg-blue-100 text-blue-700' },
                { name: 'Gate 7 Approval', file: 'signal-wave{N}-gate7-merge-approved.json', creator: 'PM', color: 'bg-purple-100 text-purple-700' },
                { name: 'Escalation', file: 'signal-wave{N}-ESCALATION.json', creator: 'Orchestrator', color: 'bg-red-100 text-red-700' },
                { name: 'L1 Approval', file: 'signal-wave{N}-L1-APPROVAL-*.json', creator: 'Human', color: 'bg-amber-100 text-amber-700' },
              ].map((s) => (
